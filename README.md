...
迷局是一款 AI 驅動的單人謀殺推理遊戲（劇本殺）。玩家扮演偵探，在有限的行動力（AP）內蒐集線索、審問嫌疑人，最終指認兇手並說明動機與手法。每局劇本由 GPT-4o 即時生成，場景、角色、兇案細節皆不重複。
...
1. 輸入偵探名字，點「確認，生成劇本」
2. 等待 AI 生成原創謀殺案（約 1 分鐘）
3. 使用 **15 點行動力（AP）** 自由搜查、審問、施壓
4. 蒐集足夠線索後點「做出最終指控」
...
| 項目 | 說明 |
|------|------|
| 技術棧 | Node.js + Express 後端、原生 HTML/CSS/JS 前端、OpenAI API |
| AI 模型 | gpt-4o（劇本生成 / GM 行動 / 結局評分） |
| 串流方式 | SSE（Server-Sent Events）實現打字機效果 |
| 遊戲時長 | 單局約 15 分鐘，15 點 AP 限制 |
...
接收玩家名稱，呼叫 GPT-4o 一次性生成完整劇本 JSON。
...
接收玩家最終指控，由 GPT-4o 評分並生成結局，一般 JSON 回應（非串流）。

- 評分項目：兇手（40）、動機（20）、手法（20）、AP 效率（10）、線索完整度（10），滿分 100
- `max_tokens`：1800
...
| `socialIssue` | theme / definition / context / killerBackground（社會議題，影響結局反思） |
| `weapon` | name / detail |
...
每局劇本包含一個社會議題（涵蓋校園霸凌、科技監控、環境破壞、職場倫理、歷史文化等五大類），融入角色動機與背景設定，在結局集中呈現。

| 階段 | 說明 |
|------|------|
| 生成階段 | socialIssue.theme / definition / context / killerBackground 由 GPT-4o 生成 |
...
| 動機分析 | 0-20 分（GPT-4o 語意評分，兇手答錯則歸零） |
| 手法推理 | 0-20 分（GPT-4o 語意評分，不論兇手對錯皆計分） |
| 調查效率（剩餘 AP） | 0-15 分（直接等於剩餘 AP 數） |
