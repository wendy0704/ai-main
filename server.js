...
- 行為描寫原則：NPC 的緊張與迴避要有具體原因（各自的秘密），不要讓所有人反應都一樣模糊；兇手可以展現特定行為矛盾（例如對某個時間點說法前後不一），但不得直接「看起來就是兇手」
- clueFound 有效性原則（核心規則）：
  ✗ 禁止：「這是兇手留下的○○」「直接證明○○是兇手」——不得讓單條線索就能鎖定兇手
  ✗ 同樣禁止：線索內容太過模糊，對推理毫無幫助（例如「地板有些灰塵」「窗戶開著」）——每條線索必須能排除或確認某個具體事實
  ✓ 應該：線索提供可解讀的具體事實片段（縮寫、時間矛盾、物品歸屬、行跡不符），讓玩家蒐集多條並交叉比對後能推論出答案
