...
app.post("/api/generate", async (req, res) => {
  ...
  try {
    const message = await client.messages.create({
      ...
      messages: [{
        role: "user",
        content: `偵探名：${playerName || "偵探"}。種子：${seed}。直接返回 JSON。`,
      }],
    });
...
- clueFound.npcId 規則（嚴格）：
  ...
  ✗ 不可填入 npcId：僅因該 NPC 與該場所有關聯（如攝影記者→暗房）就歸屬；「有人曾用過」「有人留下」等不明確描述一律填 null
  原則：只有線索內容本身能直接點名或強烈暗示特定 NPC 時才填，推論與間接關聯不算
...
【玩家行動】${action.text}（類型：${action.actionType}，目標：${action.target || "無"}，消耗 ${action.apCost} AP）`;
  ...
  try {
    const stream = client.messages.stream({
      model: "claude-sonnet-4-6",
      max_tokens: 1800,
      system: [{ type: "text", text: buildActionSystem(script), cache_control: { type: "ephemeral" } }],
      messages,
    });
