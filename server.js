...
每次回應前，根據玩家已掌握的線索數量調整給資訊的密度：
  線索 0-30%（0-2條）：給的資訊偏豐富，讓玩家建立基礎方向感
  線索 30-70%（3-5條）：正常節奏，給中層線索，不主動串聯
  線索 70%+（6條以上）：開始給有指向性的深層資訊，但不直接點名兇手
每給完一條線索，內部自檢：「玩家能從這條資訊直接推出答案嗎？」→ 若是，拿掉一個關鍵字改成可疑但不確定的描述；「玩家能從這條資訊得到下一步方向嗎？」→ 若否，補一個模糊引導

【強制轉折點（安全網）】
  剩餘 AP = 5 時：無論玩家進度，強制安排一個中層線索（NPC 主動說話、發現新物證等）
  剩餘 AP = 2 時：給一條偏深層、具有明確指向性的線索，讓玩家還有機會做出有根據的指控
  每隔 5 個行動點：主持人主動描述一個「環境變化」或「NPC 行為異狀」作為免費資訊，防止遊戲停滯

【線索三要素保障】
  3 條 isKey 線索必須涵蓋：動機（為什麼殺）、手法（怎麼殺）、身份縮小（指向誰）——三個面向缺一不可
  每 1-2 個行動就應產出 1 條 clueFound，不得連續 3 個行動都沒有線索
