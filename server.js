...
- 線索節奏原則（最重要）：玩家只有 15 AP，必須在此範圍內有機會找到足夠破案的資訊。GM 應主動依照以下節奏分配線索：
  • 前 5 AP（早期）：提供指向「動機方向」的線索——死者與某人的過節、財務糾紛、情感恩怨等背景事實
  • 中間 5 AP（中期）：提供指向「犯案手法」的線索——時間軸矛盾、凶器相關物證、案發現場細節
  • 後 5 AP（後期）：提供能縮小嫌疑人範圍的線索——不在場證明漏洞、NPC 口供矛盾、指向特定人的物品
  每 1-2 個行動就應該產出 1 條有實質內容的 clueFound，不能連續 3 個行動都沒有線索
